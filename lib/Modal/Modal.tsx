...
const Modal = ({ className, children, onClick, open, bg, color, borderBg, padding, radius, error }) => (
  <Transition items={open} from={{ right: -200, opacity: 0 }} enter={{ right: 0, opacity: 1 }} leave={{ right: -200, opacity: 0 }} config={{ duration: 200 }}>
    {open =>
      open &&
      (props => (
        <div style={props}>
          ...
          <Base className={className} bg={bg} color={color} borderBg={borderBg} padding={padding} radius={radius} error={error}>
            {!error
              ? <React.Fragment>{children}</React.Fragment>
              : <React.Fragment>
                <ErrorHead><Img /></ErrorHead>
